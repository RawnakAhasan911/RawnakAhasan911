# 👋 Hi, I'm Md. Rawnak Ahasan

### CSE Student | AI & Robotics Enthusiast | Cybersecurity | Software Development

I'm a Computer Science and Engineering student at **BRAC University**, passionate about building technology that solves real-world problems. My interests span **Artificial Intelligence, Robotics, Cybersecurity, Computer Vision, and Software Engineering**.

I enjoy working across both software and hardware , from machine learning pipelines and full-stack web applications to robotics systems and research-oriented engineering projects.

---

## 🚀 About Me

* 🎓 B.Sc. in **Computer Science & Engineering** at BRAC University
* 🤖 Interested in **AI, Robotics, Computer Vision & Intelligent Systems**
* 🔐 Exploring **Cybersecurity and secure software development**
* 💻 Experienced in **Full-Stack Development, Machine Learning & Programming**
* 🔬 Interested in **research-driven engineering and emerging technologies**
* 🧑‍🏫 Robotics Trainer at **Octobrain**
* 👥 Assistant Director at **BRAC University Computer Club**
* 🏆 Participated in national-level science, innovation, startup and engineering competitions
* 🌱 Currently expanding my skills in **AI, Robotics, Research & Modern Web Development**

---

## 🛠️ Technical Skills

### Languages

`Python` `C` `C++` `JavaScript` `HTML` `CSS` `SQL` `PHP` `Bash`

### AI / Data

`Machine Learning` `Data Analysis` `EDA` `Feature Engineering` `Python`

### Web Development

`Vue.js` `React` `Next.js` `TypeScript` `Laravel` `REST APIs` `Tailwind CSS`

### Database

`MySQL` `Google Sheets API`

### Robotics & Systems

`ROS2` `Arduino` `TurtleSim` `Docker` `Linux`

### Tools

`Git` `GitHub` `VS Code` `Canva`

---

## 🔬 Featured Projects

### 🌱 Algae Bio-Reactor for Industrial Pollution Control

**Technical Lead**

A research-oriented engineering project based on *Chlorella vulgaris*, designed to capture CO₂ and industrial pollutants while contributing to improved air quality.

🏆 **Silver Medalist , ISIF Bangladesh Round 2025**
🥉 **2nd Runner-Up , IPE Carnivesta 2025**

---

### 🤖 CampusNest

A campus-focused software project designed to provide practical digital solutions for university students.

**Tech:** JavaScript

🔗 [View Repository](https://github.com/RawnakAhasan911/CampusNest)

---

### 🚨 Emergency BD

A real-time crime reporting and visualization platform with GPS-based incident reporting and administrative moderation.

**Tech:** PHP · MySQL · JavaScript · HTML · CSS

🔗 [View Repository](https://github.com/RawnakAhasan911/Emergency-BD-CSE470)

---

### 📊 Machine Learning , Churn Prediction

An end-to-end machine learning pipeline for predicting customer subscription attrition.

Includes:

* Exploratory Data Analysis
* Missing-value handling
* Categorical encoding
* Feature engineering
* Correlation-based feature selection
* Machine learning modeling

**Tech:** Python · Jupyter Notebook

🔗 [View Repository](https://github.com/RawnakAhasan911/ML-project-on-Churn-Prediction)

---

### 🚀 Nova Siege , 3D Space Shooter

A 3D space shooter developed using **Python and PyOpenGL**, featuring custom rendering, weapon physics, and local multiplayer.

🔗 [View Repository](https://github.com/RawnakAhasan911/PyOpenGL-Space-Shooter-Game)

---

### 📝 SheezBlog

A modern full-stack blogging platform featuring:

* Role-Based Access Control
* Hybrid authentication
* OTP verification
* Authorship workflows
* Administrative security audit trails

**Tech:** Next.js · React · TypeScript · Google Sheets API

---

## 🏆 Achievements

* 🥈 **Silver Medalist** , ISIF Bangladesh Round, 2025
* 🥉 **Bronze Medalist** , Innovation World Cup Bangladesh Round, 2026
* 🥉 **2nd Runner-Up** , IPE Carnivesta, 2025
* 🚀 **Semi-Finalist** , Undergrad Startup Challenge, 2025
* 🔐 **Introduction to Cybersecurity** , Cisco Networking Academy
* 📊 **Introduction to Data Analytics** , Bohubrihi
* 🐍 **Introduction to Python Programming** , Bohubrihi
* 💻 **Pathway to Software Engineering** , Bohubrihi

---

## 💼 Experience & Leadership

**Robotics Trainer , Octobrain**
2026 – Present

Conduct hands-on robotics and STEM learning sessions while teaching programming, robotics fundamentals, and problem-solving.

**Assistant Director , BRAC University Computer Club**
2024 – Present

Contributing to technical events, competitive activities, project coordination, and club operations.

**Co-Lead, Technical Team , Team Bionergy**
2025 – Present

Working on machine development, circuit design, and system integration.

**Chief Technical Lead , Team Aronnyok**
2024 – Present

Leading research and development activities and contributing to competition-oriented technical projects.

---


## 🌐 Connect With Me

<p align="center">
  <a href="https://rawnakahasan911.github.io/raw-portfolio/">
    <img src="https://img.shields.io/badge/Portfolio-Visit%20Website-000000?style=for-the-badge&logo=googlechrome&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/ahsiuuu1306/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/RawnakAhasan911">
    <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

📊 GitHub Statistics

<table> <tr> <td align="center"> <a href="https://github.com/RawnakAhasan911"> <img height="180em" src="https://github-readme-stats.vercel.app/api?username=RawnakAhasan911&show_icons=true&theme=tokyonight&hide_border=true" /> </a> </td> <td align="center"> <a href="https://github.com/RawnakAhasan911"> <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=RawnakAhasan911&layout=compact&theme=tokyonight&hide_border=true" /> </a> </td> </tr> <tr> <td colspan="2" align="center"> <a href="https://github.com/RawnakAhasan911"> <img src="https://github-readme-streak-stats.herokuapp.com/?user=RawnakAhasan911&theme=tokyonight&hide_border=true" /> </a> </td> </tr> <tr> <td colspan="2" align="center"> <img src="https://raw.githubusercontent.com/RawnakAhasan911/RawnakAhasan911/gh-pages/github-contribution-grid-snake-dark.svg" /> </td> </tr> </table>

## 🧠 Currently Exploring

```text
Artificial Intelligence
        ↓
Computer Vision
        ↓
Robotics & ROS2
        ↓
Intelligent Autonomous Systems
        ↓
Research & Real-World Applications
```

> **Build. Learn. Experiment. Research. Repeat.**

---

<p align="center">
  <i>"They planned, but Allah also planned. And Allah is the best of planners."</i>
</p>

<p align="center">
  © 2026 Md. Rawnak Ahasan
</p>
